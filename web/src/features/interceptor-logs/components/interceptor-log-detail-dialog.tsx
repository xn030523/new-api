...
export function InterceptorLogDetailDialog(
  props: InterceptorLogDetailDialogProps
) {
    ...
    >
      <div className='space-y-4 py-2'>
        ...
        <div className='grid grid-cols-2 gap-x-6 gap-y-2 text-sm'>
          ...
          <DetailField
            label={t('Action')}
            value={isRejected ? t('Rejected') : t('Modified')}
          />
        </div>
        ...
        {isRejected && log.reject_reason && (
          <div className='space-y-1.5'>
            ...
            <div className='bg-destructive/5 border-destructive/20 relative overflow-hidden rounded-md border p-2.5'>
              <p className='text-destructive pr-6 text-xs leading-relaxed break-all whitespace-pre-wrap'>
                {log.reject_reason}
              </p>
            </div>
          </div>
        ...
        <div className='space-y-1.5'>
          ...
          <div className='bg-muted/30 relative overflow-hidden rounded-md border p-2.5'>
            ...
            />
            <p className='min-w-0 pr-6 font-mono text-xs leading-relaxed break-all whitespace-pre-wrap'>
              {log.request_body}
            </p>
          </div>
        </div>
        ...
        {!isRejected &&
          log.modified_body &&
          log.modified_body !== log.request_body && (
            <div className='space-y-1.5'>
              <label className='text-xs font-semibold text-amber-600'>
                {t('Modified Request Body')}
              </label>
              <div className='relative overflow-hidden rounded-md border border-amber-200 bg-amber-50/30 p-2.5 dark:border-amber-800 dark:bg-amber-950/10'>
                <CopyButton
                  value={log.modified_body}
                  copiedText={copiedText}
                  onCopy={copyToClipboard}
                />
                <p className='min-w-0 pr-6 font-mono text-xs leading-relaxed break-all whitespace-pre-wrap'>
...
function DetailField(props: { label: string; value: string; mono?: boolean }) {
  return (
    <div className='flex min-w-0 items-baseline gap-2'>
      ...
      <span className={`truncate text-xs ${props.mono ? 'font-mono' : ''}`}>
        {props.value}
      </span>
    </div>
  )
